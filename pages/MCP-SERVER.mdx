...
<DocsCodePreview language="bash" code={`pnpm mcp:help`} />
...
<DocsCodePreview
  language="bash"
  code={`claude mcp add --transport stdio --scope project rapidkit -- pnpm mcp:server`}
/>
...
<DocsCodePreview
  language="bash"
  code={`claude mcp add --transport stdio --scope user rapidkit -- pnpm mcp:server`}
/>
